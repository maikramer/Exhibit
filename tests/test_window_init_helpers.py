...
def _methods() -> dict[str, ast.FunctionDef]:
    ...
    for node in tree.body:
        if isinstance(node, ast.ClassDef) and node.name == "ExbWindow":
            return {
                n.name: n
                for n in node.body
                if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef))
            }
    raise AssertionError("ExbWindow missing")
...
def test_init_calls_setup_helpers():
    ...
    assert "_wire_fork_settings_widgets()" in src
    assert "_seed_primary_tab()" in src
    assert (init.end_lineno - init.lineno + 1) < 280


def test_setup_helpers_exist_and_define_actions_once():
    methods = _methods()
    assert "_setup_window_actions" in methods
    assert "_wire_fork_settings_widgets" in methods
    assert "_seed_primary_tab" in methods
...
def test_split_compare_handler_exists():
    ...
    assert "_ensure_split_compare_chrome" not in tabs
